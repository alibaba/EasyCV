...
colors = [[255,0,0], [255,255,0],[255,255,0],[0,255,255]] +[[random.randint(0, 255) for _ in range(3)] for _ in range(2000)]
...
def plot_boxes(outputs, imgs, save_path=None, color=None, line_thickness=None):
    for i in range(len(imgs)):
        ...
        if x.shape[0] != 0:
            ...
            tl = int(line_thickness or round(0.002 * (imgs[i].shape[0] + imgs[i].shape[1]) / 2)) + 1  # line/font thickness
            # tl = int(line_thickness)

            for num in range(x.shape[0]):
                c1, c2 = (int(x[num][0]), int(x[num][1])), (int(x[num][2]), int(x[num][3]))
                cv2.rectangle(imgs[i], c1, c2, colors[id[num]], thickness=tl, lineType=cv2.LINE_AA)

                tf = max(tl - 1, 1)  # font thickness
                t_size = cv2.getTextSize(label[num], 0, fontScale=tl / 10, thickness=tf)[0]

                c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
                cv2.rectangle(imgs[i], c1, c2, colors[id[num]], -1, cv2.LINE_AA)  # filled
                cv2.putText(imgs[i], label[num], (c1[0], c1[1] - 2), 0, 1, [225, 0, 255], thickness=max(tf-10,2), lineType=cv2.LINE_AA)
                cv2.putText(imgs[i], str(score[num]), (c1[0], c1[1] - 10), 0, 1, [225, 0, 255], thickness=max(tf - 10, 2),
...
def main():
    ...
    detector = TorchYoloXPredictor(output_path, model_config=config_path)
