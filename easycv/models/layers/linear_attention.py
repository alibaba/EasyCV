...
from ..utils.profiler import module_profile
...
class LinearSelfAttention(BaseLayer):
    ...
    def visualize_context_scores(context_scores):
        ...
        # [1, 1, P, N] --> [1, P, h, w]
        context_scores = context_scores.reshape(1, num_pixels, patch_h,
                                                patch_w)
        ...
        try:
            ...
            colored_context_map = cv2.applyColorMap(context_map,
                                                    cv2.COLORMAP_JET)
            ...
            f_name = '{}/h_{}_w_{}_index_'.format(res_dir_name, patch_h,
                                                  patch_w)

            files_cmap = glob('{}/h_{}_w_{}_index_*.png'.format(
    ...
    def _forward_cross_attn(self,
        ...
        query, key = torch.split(
            qk, split_size_or_sections=[1, self.embed_dim], dim=1)
        # [B, C, P, N] --> [B, d, P, N]
        value = F.conv2d(
            x,
            weight=self.qkv_proj.block.conv.weight[self.embed_dim + 1:, ...],
