...
from . import GroupLinear, LinearLayer, norm_layers_tuple
...
def initialize_fc_layer(module,
                        init_method: Optional[str] = 'normal',
                        std_val: Optional[float] = 0.01) -> None:
    """Helper function to initialize fully-connected layers"""
    if hasattr(module, 'layer'):
        _init_nn_layers(
            module=module.layer, init_method=init_method, std_val=std_val)
    else:
        _init_nn_layers(
            module=module, init_method=init_method, std_val=std_val)
...
def initialize_norm_layers(module) -> None:
    ...
    _init_fn(module.layer) if hasattr(module, 'layer') else _init_fn(
        module=module)
...
def initialize_weights(opts, modules) -> None:
    ...
    group_linear_std = getattr(opts, 'model.layer.group_linear_init_std_dev',
                               0.01)
    ...
    if isinstance(modules, nn.Sequential):
        for m in modules:
            if isinstance(m, (nn.Conv2d, nn.Conv3d)):
                ...
                initialize_fc_layer(
                    module=m,
                    init_method=linear_init_type,
                    std_val=group_linear_std)
    else:
