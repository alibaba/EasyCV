...
class ConvViTDet(FastConvMAEViT):
    ...
    def __init__(
        ...
        self.ms_adaptor = nn.ModuleList(
            [nn.Identity(), nn.Identity(), nn.Identity(), nn.MaxPool2d(2)])
    ...
    def load_pretrained(self, pretrained):
        ...
        if "rel_pos_bias.relative_position_bias_table" in unexpected_keys:
            windowed_relative_position_bias_table = resize_pos_embed(
                checkpoint["rel_pos_bias.relative_position_bias_table"][None, :-3],
                self.windowed_rel_pos_bias.relative_position_bias_table[None], 0,)
            ...
            self.windowed_rel_pos_bias.load_state_dict(
                {"relative_position_bias_table": windowed_relative_position_bias_table[0]},
                strict=False)
